...
export async function getSnapshot(): Promise<Snapshot> {
  if (!configured()) {
    return {
      ...demoSnapshot(), shelters: [], hazards: [], demo: false,
      setupRequired: true, syncedAt: new Date().toISOString(),
    };
  }
  ...
  if (data.demo) return { ...data, shelters: [], hazards: [], demo: false, setupRequired: true } as Snapshot;
